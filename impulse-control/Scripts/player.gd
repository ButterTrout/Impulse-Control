...
## Weight of gravity
@export var fall_acceleration : float = 14
## How fast do we run?
...
func _physics_process(delta: float) -> void:
	...
	if can_jump:
		if Input.is_action_pressed(input_jump) and is_on_floor():
			...
			can_jump   = false
	# Implement jump cancel
	else:
		if Input.is_action_just_released(input_jump) and velocity.y > 0.0 and not is_on_floor():
			velocity.y = 0.0
		if is_on_floor():
			can_jump = true
...
func get_collisions():
	...
	if raycast.is_colliding() and hit.has_method("interact"):
		return [true,hit]
	else:
		return [false,null]
...
func _on_area_3d_area_exited(area: Area3D) -> void:
	var temp = current_areas.find(area)
	current_areas.remove_at(temp)
	pass # Replace with function body.
